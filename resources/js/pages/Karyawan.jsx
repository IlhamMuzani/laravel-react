...
export default function Karyawan() {
    ...
    useEffect(() => {
        fetch("http://localhost/backend-app/api/karyawan")
   ...
   const handleSubmit = async () => {
    ...
    try {
        const url = isEditing
            ? `http://localhost/backend-app/api/karyawan/${formData.id}`
            : "http://localhost/backend-app/api/karyawan/add";
...
const handleDelete = async (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`http://localhost/backend-app/api/karyawan/${id}`, { method: "DELETE" });
