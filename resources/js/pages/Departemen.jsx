...
export default function Departemen() {
    ...
    useEffect(() => {
        fetch("http://localhost/backend-app/api/departemen")
   ...
   const handleSubmit = async () => {
    ...
    try {
        const url = isEditing
            ? `http://localhost/backend-app/api/departemen/${formData.id}`
            : "http://localhost/backend-app/api/departemen/add";
...
const handleDelete = async (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`http://localhost/backend-app/api/departemen/${id}`, { method: "DELETE" });
